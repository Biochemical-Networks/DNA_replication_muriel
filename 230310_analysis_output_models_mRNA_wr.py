...
def graph_error_g_bb_multiple_models_per_model4(L_delta_g_bb, L_epsilon, model, delta_g_tt, model_title, L_std_high, L_std_low, color_def, line_style):
    plt.plot(L_delta_g_bb, L_epsilon, label = "$\Delta G_{tt}$ = %s, %s" %(delta_g_tt, model_title[12:]), linestyle=line_style, color= color_def)
    plt.fill_between(L_delta_g_bb, L_std_low, L_std_high, color= color_def, alpha=0.1)
    plt.title("%s: Effect $\Delta G_{pol}$ on error probability of the polymer string" %(model_title[0:12]))
    plt.xlabel("Backbone strength $\Delta G_{pol}$")
    plt.ylabel("Error probability $\epsilon$")
    plt.legend(bbox_to_anchor=(1.04, 1))
    plt.subplots_adjust(right=0.7)
...
date_options = ["2023310", "2023311", "2023210", "2023211"]
...
L_models_titles = [L_models_titles[i], L_models_titles[i+1]] #(i=0,2,4 6, 8,10,12) ,2-8-12
...
# L_models_titles = [L_models_titles[2], L_models_titles[10]]#
# L_delta_g_tt = [4]
...
for k, model in enumerate(L_models):
    ...
    for j, delta_g_tt in enumerate(L_delta_g_tt):
        ...
        # graph_error_g_bb_multiple_models_per_model3(L_delta_g_bb_graph, L_epsilon_mean, model, delta_g_tt, L_models_titles[k], L_epsilon_std_high, L_epsilon_std_low, color_list[j*40], line_styles[k])
        # graph_error_g_bb_multiple_models_per_model4(L_delta_g_bb_graph, L_epsilon_mean, model, delta_g_tt, L_models_titles[k], L_epsilon_std_high, L_epsilon_std_low, color_list[j*10], line_styles[k])
