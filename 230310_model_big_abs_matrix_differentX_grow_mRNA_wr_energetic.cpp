...
string declare_filename(bool all_steps, string DIR, string model_1, double delta_g_pol, double delta_g_tt, int step){
    ...
    if(all_steps ==true){
        ...
        filename_output = path + date_now + "model_muriel_big_abs_" + model_1 + "_delta_g_pol_" + str_rounded_delta_g_pol + "_delta_g_tt_" + std::to_string(rounded_delta_g_tt) + "_allsteps.csv";
    }
...
string declare_filename_parameter(bool all_steps, string DIR, string M_model, string x_def){
    ...
    string path = "/home/ipausers/louman/Documents/programming/DNA_replication_muriel/outs/" + DIR + "/";
    if(all_steps ==true){
        filename_output = path + date_now + "parameter_file_model_big_abs_" + M_model + "_x" + x_def + "_allsteps.csv";}
    else{
